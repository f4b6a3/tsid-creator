...
public class TsidTest {
	...
	public void testFromStringFormat() {
		...
		for (int i = 0; i < string.length; i++) {
			...
			String upper = head + tsid.toString() + tail;
			String lower = head + tsid.toLowerCase() + tail;

			assertEquals(tsid, Tsid.from(upper, format));
			assertEquals(tsid, Tsid.from(lower, format));
		}
	...
	public void testToStringFormat() {
		...
		for (int i = 0; i < string.length; i++) {
			...
			assertEquals(upper, tsid.toString(format));
			assertEquals(lower, tsid.toLowerCase(format));
		}

		try {
			tsid.toString((String) null);
			fail();
		} catch (IllegalArgumentException e) {
			// success
		}
	...
	@Test
	public void testToAndFromStringFormat() {
		...
		for (int i = 0; i < string.length; i++) {
			...
			assertEquals(upper, Tsid.from(upper, format).toString(format));
			assertEquals(lower, Tsid.from(lower, format).toLowerCase(format));

			assertEquals(tsid, Tsid.from(tsid.toString(format), format));
			assertEquals(tsid, Tsid.from(tsid.toLowerCase(format), format));
		}
	}
